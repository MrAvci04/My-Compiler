...
extern  int LastSTAddress;
extern map<string, int> myST;
extern map<std::string, int> sizeST;
extern bool isCurrConst;
extern bool checkCurr;
extern int curr;
extern bool fromBinOp;
extern bool fromBinOp2;
extern int conCnt;
extern int lpCnt;
extern bool fromFunc;
extern double dcurr;
extern bool iOd;
extern int currg;
extern int csnm[];
extern int temp;
extern bool fromCase;
extern bool isRecType;
extern bool checkType;
extern int currSize;
extern bool fromRecList;
extern bool fromVD;
extern int VDSize;
...
public:
  ...
  void pcodegen(ostream& os) {
      isCurrConst = true;
      //if(!checkCurr) // why?! we should write !fromBin
...
public :
  ...
  void pcodegen(ostream& os) {
      ...
      os << "case_" << csnm[ch - 1] << "_" << ch << ":" << endl;
      csnm[ch - 1]++;
      stat_list_->pcodegen(os);

      os << "ujp end_switch_" << ch << endl;
      currg = ch;
  }
...
public :
  ...
  void pcodegen(ostream& os) {
      ...
      os << "loop_" << myId << ":" << endl;
      exp_->pcodegen(os);
      os << "fjp end_loop_" << myId << endl;
      stat_list_->pcodegen(os);
      os << "ujp end_loop_" << myId <<endl;
      os << "end_loop_" << myId <<":"<< endl;
  }
...
public :
  ...
  void pcodegen(ostream& os) {
      ...
      int myId = conCnt++;
      if(!stat_list_else_)
      os << "fjp end_if_" << myId << endl;
      else
          os << "fjp else_if_" << myId << endl;
      stat_list_if_->pcodegen(os);
     // if (!stat_list_else_)
      //os << "ujp end_if_" << myId << endl;
      if (stat_list_else_) {
          os << "ujp end_if_" << myId << endl;
          os << "else_if_" << myId << endl;
          stat_list_else_->pcodegen(os);
      }
      os << "end_if_" << myId << endl;
  }
...
public :
  ...
  void print (ostream& os) {
		os<<"Node name : Assign";
		assert(var_ && exp_);
    exp_->print(os);//Node name : Expr    Unary op is : 287 -> var=-
    var_->print(os);//Node name : IntConst. Value is :1 -> exp=1
  }
  void pcodegen(ostream& os) {
      assert(var_ && exp_);
      exp_->pcodegen(os);
...
public:
  ...
  void pcodegen(ostream& os) {
      ...
      // print ldc with the address from the symbol table
      //os << "simple type" << endl;
  }
...
public:
  ...
  void pcodegen(ostream& os) {
      os << "ldc " << myST[*name_] <<*name_ << endl;
      if (fromBinOp || fromFunc || fromCase)
          os << "ind" << endl;
      if (fromVD)
          VDSize = 1;
  }
...
public:
  ...
  void pcodegen(ostream& os) {
      ...
      fromVD = true;
      type_->pcodegen(os);
      myST.insert(pair<string ,int>(*name_,LastSTAddress+VDSize));
     // os << *name_ << endl;
      LastSTAddress += VDSize;
      sizeST.insert(pair<string, int>(*name_, VDSize));
      fromVD = false;
  }
